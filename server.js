...
- LANGUAGE RULE: Match the user's exact language and script. 
  * If they ask in pure Hindi (Devanagari script), reply in pure Hindi. 
  * If they ask in Hinglish (Hindi words written in English letters, e.g., "Kaise ho"), reply in Hinglish. 
  * If they ask in English, reply in English. 
  NEVER reply in English if the user asked in Hindi/Hinglish.
...
7. LANGUAGE RULE: Match the user's exact language (English, Hindi, or Hinglish). If they write Hindi in English letters (Hinglish), you MUST reply in Hinglish.
...
QUESTION: "${query}" (Note: Question may be in Hindi/Hinglish, match concepts accordingly)
...
- Minor word choice/paraphrasing/language translation is fine.
...
function classifyQuery(message, conversationHistory = []) {
  try {
    ...
    // Expanded pronouns to correctly map Hinglish follow-ups
    const hasPronoun = /\b(it|this|that|they|him|her|those|iska|isko|uska|usko|unka|unhone|isne|inhe|inhone|ye|wo)\b/i.test(queryLower);
    // Expanded follow-up words to include Hinglish/Hindi
    const startsWithFollowupWord = /^(and|but|also|what about|tell me more|explain|why|how|elaborate|go deeper|give me more|what else|aur|par|lekin|kyun|kaise|batao)\b/i.test(queryLower);
    ...
    // Added common Hindi spiritual terms to keep routing accurate
    const kbTerms = [
      ...
      'bhagwan', 'parmatma', 'allah', 'khuda', 'rab', 'ishwar', 'gyan', 'bhakti',
      'paap', 'punya', 'swarg', 'narak', 'dharm'
    ];
...
function expandQueryLocal(originalQuery) {
  ...
  // Added massive Hinglish dictionary stopword block to ensure it extracts the right entities
  const entityOnly = workQ
    .replace(/\b(what|who|how|why|when|where|which|is|are|was|were|did|does|do|can|tell|me|about|the|a|an|of|in|from|for|to|and|or|by|with|this|that|it|those|these|kya|kyu|kyun|kaise|kaun|kahan|kab|kitna|kisne|kisko|kiski|kiska|ki|ke|ka|ko|se|mein|par|aur|ya|hai|hain|tha|thi|the|hun|ho|batao|samjhao|dikhao|ye|wo|is|us|in|un|bhi|hi|to)\b/gi, '')
    .replace(/[?.,!]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
...
function isBroadQuery(query) {
  // Added Hindi variants to capture broad queries accurately
  return /^(how many|what is|explain|describe|what are|who is|tell me about|list|kya hai|kaise|kitne|kaun|kisne|kaha|kaise bana|kya hota hai|body layers|souls layers)/i.test(query.trim());
}
...
function extractQueryEntities(query) {
  // Added hinglish Stopwords to avoid extracting them as "entities"
  const hinglishStopwords = new Set(['kya', 'hai', 'kaise', 'kyun', 'kab', 'kahan', 'kaun', 'kitna', 'kisne', 'kisko', 'kiski', 'kiska', 'ki', 'ke', 'ka', 'ko', 'se', 'mein', 'par', 'aur', 'ya', 'hain', 'tha', 'thi', 'the', 'hun', 'ho', 'ye', 'wo', 'is', 'us', 'in', 'un', 'bhi', 'hi', 'to']);
  return query.toLowerCase()
    .replace(/[?.,!\"']/g, '')
    .split(/\s+/)
    .filter(w => w.length > 2 && !STOPWORDS.has(w) && !hinglishStopwords.has(w));
}
...
// ═══════════════════════════════════════════════════════
async function searchQABank(originalQuery, llmExpandedQueries = []) {
  if (!chromaQACollection) return { qaChunks: [], qaTopScore: 0 };

  try {
    const localQueries = expandQueryLocal(originalQuery);
    // Merge LLM generated English translation queries with local ones
    const queries = [...new Set([...localQueries, ...llmExpandedQueries])].slice(0, 12);
...
// ═══════════════════════════════════════════════════════
async function searchKBChunks(originalQuery, llmExpandedQueries = []) {
  if (!chromaReady || !chromaCollection) return { kbChunks: [], kbTopScore: 0 };

  try {
    const localQueries = expandQueryLocal(originalQuery);
    // Merge LLM English Translations with local queries
    const queries = [...new Set([...localQueries, ...llmExpandedQueries])].slice(0, 15);
...
// Classifies query type, extracts key requirements, and generates english translation vector searches.
// ═══════════════════════════════════════════════════════
...
Return JSON matching this schema:
{
  ...
  "search_queries": ["string"], // MANDATORY: Provide 1 to 3 optimized ENGLISH search queries for vector database retrieval. If the user query is in Hindi or Hinglish, accurately translating the core concepts into English here is CRITICAL.
...
Question: "${query}" (Note: May be in Hindi/Hinglish, validate against English passages appropriately)
...
async function searchDatabase(originalQuery) {
  ...
  try {
    ...
    const llmExpanded = intent.search_queries || [];
    console.log(`[Intent] English translation searches generated: ${llmExpanded.join(', ')}`);
    ...
    if (intent.needs_decomposition && intent.sub_queries && intent.sub_queries.length > 0) {
      ...
      const qaResults = await Promise.all(subQueries.map(q => searchQABank(q, llmExpanded)));
      const kbResults = await Promise.all(subQueries.map(q => searchKBChunks(q, llmExpanded)));
      ...
      // Normal retrieval passing the LLM generated English Search queries
      const qaRes = await searchQABank(originalQuery, llmExpanded);
      qaChunks = qaRes.qaChunks;
      qaTopScore = qaRes.qaTopScore;

      const kbRes = await searchKBChunks(originalQuery, llmExpanded);
...
app.get('/api/test-thresholds', async (req, res) => {
  ...
  for (const q of queries) {
    try {
      ...
      if (intent.needs_decomposition && intent.sub_queries && intent.sub_queries.length > 0) {
        const subQueries = intent.sub_queries.slice(0, 3);
        const qaResults = await Promise.all(subQueries.map(sq => searchQABank(sq, llmExpanded)));
        const kbResults = await Promise.all(subQueries.map(sq => searchKBChunks(sq, llmExpanded)));
        ...
        const qaRes = await searchQABank(q, llmExpanded);
        qaChunks = qaRes.qaChunks;
        qaTopScore = qaRes.qaTopScore;

        const kbRes = await searchKBChunks(q, llmExpanded);
        kbChunks = kbRes.kbChunks;
        kbTopScore = kbRes.kbTopScore;
      }
